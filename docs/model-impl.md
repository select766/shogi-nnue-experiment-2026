...
- **OOMバグ:** `NNUEExperts.forward()` の einsum による重みブレンドが~30GB中間テンソルを生成。per-expert forward + 出力レベル加重平均に変更してOOM回避。
...
**解決:** per-expert forward pass に変更し、出力レベルで加重平均する方式を採用。各 expert の NNUE 評価値を個別に計算し、`(batch, n_experts)` テンソルを gate_weights で加重和。メモリ使用量が大幅削減。
...
| リスク | 影響度 | 対策 | 状態 |
|--------|--------|------|------|
| NNUE experts × N の重みが大きくGPUメモリ不足 | 中 | N_EXPERTS=4から開始。出力レベル加重平均でOOM回避 | 解決済み |
