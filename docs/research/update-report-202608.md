...
> 2026-09-20更新: 本文は8月の経過を保存しており、過去の「次の実験」も含む。
> 現在の判断は[研究レビュー](results/research-review-20260920/README.md)、
> 次に実行する条件は[次期実験計画](experiments/research-next-20260920.md)、
> 仮説の状態と優先順位は[台帳](hypotheses.md)を参照する。
> decision-alignedの14,000局はペア再解析でも約+6.7 Eloだが、対局履歴・キャッシュ条件と
> データ分離を明示した独立再現を優先する。

## 結論

- 浅いroot探索統計モデルを静的7特徴対照と固定10万nodesで4,000局比較すると、1,921勝1,942敗
  137分、`-1.82 Elo`、95%区間`[-12.41,+8.76]`だった。validation bestmove `+0.09`ptは独立testで
  再現せず、自己対局でも棋力転換を確認できなかった。点推定が負なので同候補の局数は拡張せず、
  過去にbestmoveとEloが一貫して正だったdecision-aligned目的を12,000局へ深掘りする。
...
- 実探索leaf候補は250、1,000、4,000、16,000、100万nodesの全leaf分布でcheckpoint 510より
  group lossを改善し、100万nodesでも差`-0.00384`、95%区間`[-0.00432,-0.00340]`だった。
  一方、固定test 10,000局面の最善手一致率は候補62.58%、control 62.78%でMcNemar
  `p=0.652110`、10万nodes自己対局400局は候補200勝185敗15分、Elo `+13.0`
  `[-20.4,+46.5]`だった。探索量転移は支持されたが、棋力改善は確認できない。詳細は
  `docs/research/results/search-budget-and-strength-20260824/README.md`に記録した。
- 同じ自己対局を固定4,000局まで追試すると1,950勝1,919敗131分、Elo `+2.69`、95%区間
  `[-7.90,+13.28]`となった。区間は縮小したが0を跨ぎ、leaf loss改善から棋力向上は確認できなかった。
  今後はleaf平均KL単独で選抜せず、rootの候補手順位や探索utilityに整合した目的を優先検証する。詳細は
  `docs/research/results/leaf-loss-strength-20260826/README.md`に記録した。
...
## Joint候補方向とroot統計棋力 (2026-08-28)

複数expertを同時に動かす14本のsigned Hadamard方向を一軸8方向へ追加し、fresh 768 rootを
selection/held-out各384へ固定分割した。selectionではjoint方向4本を含む8候補が有効教師率、gain、
候補手多様度をすべて改善したが、held-outでは有効教師率差0、gain差`-37.625cp`、多様度差
`-0.0052`となった。`H-SU-JOINT-DIRECTIONS`は同時改善が転移しないため棄却した。

浅いroot探索統計候補を固定10万nodesで静的7特徴対照と4,000局対局させた。結果は1,921勝
1,942敗137分、`-1.82 Elo`、95%区間`[-12.41,+8.76]`で、validationの小さなbestmove改善は
棋力へ転換しなかった。浅い1024-node探索の追加時間は本探索比0.528%だった。

## 24時間 深掘り検証 (2026-08-28)

一次仮説が棄却または棋力で負方向だったため、事前計画どおり過去の
`H-DECISION-ALIGNED-LOSS`を深掘りした。既存4,000局と重ならないtest由来4,000開始局面を
先後反転した8,000局を追加した。追加単独は3,935勝3,781敗284分、`+6.69 Elo`、95%区間
`[-0.79,+14.17]`、合計12,000局は5,917勝5,679敗404分、`+6.89 Elo`、95%区間
`[+0.78,+13.00]`だった。事前成功条件の合算区間下端`>0`を満たしたため、固定条件の小効果として
`H-DECISION-ALIGNED-LOSS`を支持へ更新した。

支持判定を固定後、残り時間で未使用test開始局面1,000件を先後反転した2,000局を事前登録して
感度追試した。単独は985勝954敗61分、`+5.39 Elo`、95%区間`[-9.61,+20.38]`、合計14,000局は
6,902勝6,633敗465分、`+6.68 Elo`、95%区間`[+1.02,+12.34]`だった。追加標本の点推定も正で、
12,000局の支持判定と効果量は安定した。

残り時間には重複なし1,000局面で浅いroot統計の固定bestmoveを追加追試した。静的7特徴65.60%に
対して候補62.90%、差`-2.70`ポイント、McNemar `p=0.038878`だった。既存独立testの`-0.01`
ポイントと合わせ、validation改善は独立splitへ転移しないため`H-ROOT-SEARCH-STATS`を棄却へ更新した。
24時間枠内に全ジョブ、台帳検査、テスト、コミットまで完了し、未完了ジョブは残していない。
