#!/bin/bash
# Expert Blending v4 (8 experts, paired, uniform_50 dataset, NNUE backbone) entry script.
# Hyperparameters are fixed in this script; common resume logic is delegated.
#
# Usage: bash scripts/run_train_expert_blending_8experts_v4_paired_uniform50_nnue_backbone_noise0.sh
set -euo pipefail

SCRIPT_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
LOGDIR="${SCRIPT_ROOT}/logs/expert_blending_8experts_v4_paired_uniform50_nnue_backbone_noise0"
LOG_FILE="/tmp/train_expert_blending_8experts_v4_paired_uniform50_nnue_backbone_noise0.log"
