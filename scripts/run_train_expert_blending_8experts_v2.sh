#!/bin/bash
# Expert Blending モデル (8 experts) の長時間学習スクリプト。
# dataset/split_v1/ (269GB train.bin) を使用する。
# Stop with Ctrl-C, then re-run to resume from latest checkpoint.
#
# Usage: bash scripts/run_train_expert_blending_8experts.sh
set -e

SCRIPT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
NNUE_PYTORCH_DIR="${SCRIPT_DIR}/nnue-pytorch"
LOGDIR="${SCRIPT_DIR}/logs/expert_blending_8experts_v2"
CKPT_DIR="${LOGDIR}/checkpoints"
LOG_FILE="/tmp/train_expert_blending_8experts_v2.log"
