#!/usr/bin/env bash
# Fixed 4,000-game strength test for online shallow-root statistics.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORK="${REPO_ROOT}/tmp/root_search_stats_strength_match"
...
if ! flock -n 9; then
    echo "Another root-search-statistics match is already running" >&2
    exit 2
fi
...
"${REPO_ROOT}/scripts/project_python.sh" \
    ...
    --output "$OPENINGS" --count 2000 --start 2200 \
...
run_chunk() {
    local index="$1"
    local result="${REPO_ROOT}/results/match_root_search_stats_chunk_${index}.json"
    if [[ -s "$result" ]]; then
...
for index in $(seq -w 0 39); do
    if [[ -s "${REPO_ROOT}/results/match_root_search_stats_chunk_${index}.json" ]]; then
        continue
    fi
...
"${REPO_ROOT}/scripts/project_python.sh" \
    ...
    --input "${results[@]}" --expected-games 4000 --require-color-pairs \
    --output "${REPO_ROOT}/results/match_root_search_stats_4000.json"
