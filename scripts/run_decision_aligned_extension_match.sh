#!/usr/bin/env bash
# Add 8,000 fresh games and combine them with the preregistered 4,000-game run.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WORK="${REPO_ROOT}/tmp/decision_aligned_extension_match"
OPENINGS="${WORK}/openings_4000.jsonl"
...
if ! flock -n 9; then
    echo "Another decision-aligned extension match is already running" >&2
    exit 2
fi
"${REPO_ROOT}/scripts/project_python.sh" \
    ...
    --output "$OPENINGS" --count 4000 --start 0 --seed 20260827 \
    --max-abs-eval 500
...
run_chunk() {
    local index="$1"
    local result="${REPO_ROOT}/results/match_decision_aligned_extension_chunk_${index}.json"
    if [[ -s "$result" ]]; then
...
for index in $(seq -w 0 79); do
    if [[ -s "${REPO_ROOT}/results/match_decision_aligned_extension_chunk_${index}.json" ]]; then
        continue
    fi
...
"${REPO_ROOT}/scripts/project_python.sh" \
    ...
    --input "${results[@]}" --expected-games 12000 --require-color-pairs \
    --output "${REPO_ROOT}/results/match_decision_aligned_12000.json"
