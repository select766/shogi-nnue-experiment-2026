"""Optimize one gate distribution shared by every leaf in a root group."""

import argparse
import json
import os
from pathlib import Path

import numpy as np
import torch

import features as nnue_features
from train_nnue.diagnose_gate import infer_model_configuration, per_record_loss
from train_nnue.expert_blending_model import create_expert_blending_model
from train_nnue.root_grouped_dataset import RootGroupedDataset


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--data", required=True)
    parser.add_argument("--backbone-weights", required=True)
    parser.add_argument("--nnue-checkpoint", required=True)
    parser.add_argument("--max-roots", type=int, required=True)
    parser.add_argument("--root-batch-size", type=int, default=256)
    parser.add_argument("--steps", type=int, default=10)
    parser.add_argument("--lr", type=float, default=0.1)
    parser.add_argument("--prior-kl", type=float, default=0.01)
    parser.add_argument("--output", required=True)
    parser.add_argument("--device", default="cuda:0")
    args = parser.parse_args()
    if not args.device.startswith("cuda") or not torch.cuda.is_available():
        raise RuntimeError("root-grouped teacher generation requires CUDA")
    if args.max_roots <= 0 or args.steps <= 0 or args.lr <= 0.0:
        parser.error("root/step counts and lr must be positive")
    if args.prior_kl < 0.0:
        parser.error("prior-kl must be non-negative")

    checkpoint = torch.load(args.checkpoint, map_location="cpu", weights_only=False)
    configuration = infer_model_configuration(checkpoint["state_dict"])
    hyperparameters = checkpoint.get("hyper_parameters", {})
    feature_set = nnue_features.get_feature_set_from_name("HalfKP")
    model = create_expert_blending_model(
        backbone_weights_path=args.backbone_weights,
        nnue_ckpt_path=args.nnue_checkpoint,
        feature_set=feature_set,
        n_experts=configuration["n_experts"],
        adapter_hidden=configuration["adapter_hidden"],
        backbone_type=configuration["backbone_type"],
        blend_mode=configuration["blend_mode"],
        device="cpu",
    )
    model.load_state_dict(
        {
            key.removeprefix("model."): value
            for key, value in checkpoint["state_dict"].items()
            if key.startswith("model.")
        }
    )
    model.requires_grad_(False)
    model.to(args.device).eval()
    n_experts = model.nnue_experts.n_experts
    dataset = RootGroupedDataset(
        args.data,
        "HalfKP",
        args.root_batch_size,
        device=args.device,
    )
    if args.max_roots > dataset.num_roots:
        parser.error("max-roots exceeds the grouped dataset")
    group_size = dataset.group_size
    objective = {
        "score_scaling": float(hyperparameters.get("score_scaling", 361.0)),
        "lambda": float(hyperparameters.get("lambda_", 1.0)),
        "label_smoothing_eps": float(
            hyperparameters.get("label_smoothing_eps", 0.0)
        ),
    }

    output_path = Path(args.output)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    temporary_path = Path(f"{output_path}.tmp.npy")
    output = np.lib.format.open_memmap(
        temporary_path,
        mode="w+",
        dtype=np.float32,
        shape=(args.max_roots, 2 * n_experts),
    )
    processed = 0
    initial_sum = 0.0
    optimized_sum = 0.0
    entropy_sum = 0.0
    iterator = iter(dataset)
    while processed < args.max_roots:
        x1, x2, us, them, white, black, outcome, score, _ = next(iterator)
        if white.is_sparse:
            white = white.to_dense()
        if black.is_sparse:
            black = black.to_dense()
        root_count = int(x1.shape[0])
        with torch.no_grad():
            features = model.backbone(x1, x2)
            initial_weights = model.adapter(features, training=False)
            expert_values = torch.cat(
                [
                    model.nnue_experts.forward_expert(
                        index, us, them, white, black
                    )
                    for index in range(n_experts)
                ],
                dim=1,
            )
            leaf_expert_losses = torch.stack(
                [
                    per_record_loss(
                        expert_values[:, index : index + 1],
                        score,
                        outcome,
                        objective["score_scaling"],
                        objective["lambda"],
                        objective["label_smoothing_eps"],
                    )
                    for index in range(n_experts)
                ],
                dim=1,
            )
            grouped_expert_losses = leaf_expert_losses.reshape(
                root_count, group_size, n_experts
            ).mean(dim=1)
            initial_value = model.nnue_experts(
                initial_weights.repeat_interleave(group_size, dim=0),
                us,
                them,
                white,
                black,
            )
            initial_group_loss = per_record_loss(
                initial_value,
                score,
                outcome,
                objective["score_scaling"],
                objective["lambda"],
                objective["label_smoothing_eps"],
            ).reshape(root_count, group_size).mean(dim=1)

        logits = initial_weights.clamp_min(1e-12).log().detach().requires_grad_(True)
        optimizer = torch.optim.Adam([logits], lr=args.lr)
        for _ in range(args.steps):
            optimizer.zero_grad()
            weights = torch.softmax(logits, dim=-1)
            raw_value = model.nnue_experts(
                weights.repeat_interleave(group_size, dim=0),
                us,
                them,
                white,
                black,
            )
            group_loss = per_record_loss(
                raw_value,
                score,
                outcome,
                objective["score_scaling"],
                objective["lambda"],
                objective["label_smoothing_eps"],
            ).reshape(root_count, group_size).mean(dim=1)
            prior = (
                weights
                * ((weights + 1e-12).log() - (initial_weights + 1e-12).log())
            ).sum(dim=-1)
            (group_loss + args.prior_kl * prior).mean().backward()
            optimizer.step()

        with torch.no_grad():
            optimized_weights = torch.softmax(logits, dim=-1)
            optimized_value = model.nnue_experts(
                optimized_weights.repeat_interleave(group_size, dim=0),
                us,
                them,
                white,
                black,
            )
            optimized_group_loss = per_record_loss(
                optimized_value,
                score,
                outcome,
                objective["score_scaling"],
                objective["lambda"],
                objective["label_smoothing_eps"],
            ).reshape(root_count, group_size).mean(dim=1)
            entropy = -(
                optimized_weights * (optimized_weights + 1e-12).log()
            ).sum(dim=-1)
        take = min(root_count, args.max_roots - processed)
        output[processed : processed + take, :n_experts] = (
            grouped_expert_losses[:take].cpu().numpy()
        )
        output[processed : processed + take, n_experts:] = (
            optimized_weights[:take].cpu().numpy()
        )
        initial_sum += float(initial_group_loss[:take].sum().cpu())
        optimized_sum += float(optimized_group_loss[:take].sum().cpu())
        entropy_sum += float(entropy[:take].sum().cpu())
        processed += take
        if processed % 10000 < take:
            print(f"Progress: {processed}/{args.max_roots}", flush=True)
    output.flush()
    del output
    os.replace(temporary_path, output_path)
    summary = {
        "format": "root-grouped-teacher-v1",
        "checkpoint": os.path.abspath(args.checkpoint),
        "data": os.path.abspath(args.data),
        "roots": args.max_roots,
        "group_size": group_size,
        "n_experts": n_experts,
        "steps": args.steps,
        "lr": args.lr,
        "prior_kl": args.prior_kl,
        "objective": objective,
        "initial_mean_group_loss": initial_sum / processed,
        "optimized_mean_group_loss": optimized_sum / processed,
        "optimized_mean_entropy": entropy_sum / processed,
    }
    with output_path.with_suffix(".json").open("w") as file:
        json.dump(summary, file, indent=2)
        file.write("\n")
    print(json.dumps(summary, indent=2))


if __name__ == "__main__":
    main()
