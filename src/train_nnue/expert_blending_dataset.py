...
1) Legacy format: 40 bytes/record (single PackedSfenValue)
2) Paired format: 80 bytes/record
   [DNN PackedSfenValue (40B) | NNUE PackedSfenValue (40B)]
...
RECORD_BYTES = 40  # backward compatibility
LEGACY_RECORD_BYTES = 40
PAIRED_RECORD_BYTES = 80
PSFEN_BYTES = 32
PAIRED_DNN_OFFSET = 0
PAIRED_NNUE_OFFSET = 40
...
class ExpertBlendingDataset(IterableDataset):
    ...
    def __init__(self, bin_path, feature_set_name, batch_size, device='cpu', shuffle=True):
        ...
        self.num_records = os.path.getsize(bin_path) // LEGACY_RECORD_BYTES
        self.feature_set = nnue_features.get_feature_set_from_name(feature_set_name)
        self.nnue_bin_path = bin_path
...
def _paired_nnue_cache_path(paired_bin_path, cache_dir=None):
    paired_path = Path(paired_bin_path)
    if cache_dir:
        cache_base = Path(cache_dir)
        cache_base.mkdir(parents=True, exist_ok=True)
    else:
        cache_base = paired_path.parent
    return str(cache_base / f"{paired_path.name}.nnue40.bin")


def extract_paired_nnue_bin(paired_bin_path, output_path=None, cache_dir=None, chunk_records=262144):
    """Extract NNUE-side 40B records from 80B paired records.

    Output is a standard 40B/record packed SFEN file that SparseBatchProvider can read.
    Reuses an existing valid cache file when present.
    """
    src_size = os.path.getsize(paired_bin_path)
    if src_size % PAIRED_RECORD_BYTES != 0:
        raise ValueError(
            f"Paired bin size must be a multiple of {PAIRED_RECORD_BYTES}: {paired_bin_path}"
        )
    num_records = src_size // PAIRED_RECORD_BYTES
    expected_out_size = num_records * LEGACY_RECORD_BYTES

    out_path = output_path or _paired_nnue_cache_path(paired_bin_path, cache_dir=cache_dir)
    if os.path.exists(out_path) and os.path.getsize(out_path) == expected_out_size:
        return out_path

    tmp_path = f"{out_path}.tmp.{os.getpid()}"
    with open(paired_bin_path, "rb") as src, open(tmp_path, "wb") as dst:
        while True:
            block = src.read(chunk_records * PAIRED_RECORD_BYTES)
            if not block:
                break
            if len(block) % PAIRED_RECORD_BYTES != 0:
                raise ValueError(f"Corrupted paired bin block in {paired_bin_path}")
            arr = np.frombuffer(block, dtype=np.uint8).reshape(-1, PAIRED_RECORD_BYTES)
            dst.write(arr[:, PAIRED_NNUE_OFFSET:PAIRED_NNUE_OFFSET + LEGACY_RECORD_BYTES].tobytes())
        dst.flush()
        os.fsync(dst.fileno())
...
class PairedExpertBlendingDataset(IterableDataset):
    """80B/record のペア形式データセット。

    - DNN特徴量: 各レコード前半40B (qsearch未適用)
    - NNUE特徴量/教師信号: 各レコード後半40B (qsearch適用済み)
    """
    ...
    def __init__(
        ...
        if bin_size % PAIRED_RECORD_BYTES != 0:
            raise ValueError(
                f"Paired bin size must be a multiple of {PAIRED_RECORD_BYTES}: {bin_path}"
            )
        self.num_records = bin_size // PAIRED_RECORD_BYTES
        self.feature_set = nnue_features.get_feature_set_from_name(feature_set_name)
        self.record_bytes = PAIRED_RECORD_BYTES
        self.dnn_psfen_offset = PAIRED_DNN_OFFSET
        self.nnue_bin_path = nnue_bin_path or extract_paired_nnue_bin(
            bin_path, cache_dir=nnue_cache_dir
        )
...
def create_data_loaders(
    ...
    """
    dataset_cls = PairedExpertBlendingDataset if paired else ExpertBlendingDataset
    dataset_kwargs = {"device": device}
    if paired:
        dataset_kwargs["nnue_cache_dir"] = nnue_cache_dir
    ...
    val_record_bytes = PAIRED_RECORD_BYTES if paired else LEGACY_RECORD_BYTES
    val_records = os.path.getsize(val_bin) // val_record_bytes
...
if __name__ == "__main__":
    ...
    parser.add_argument("--paired", action="store_true", help="Use 80B paired records")
    parser.add_argument(
        "--nnue-cache-dir",
        default=None,
        help="Directory for extracted NNUE-side .bin cache in paired mode",
    )
    ...
    dataset_cls = PairedExpertBlendingDataset if args.paired else ExpertBlendingDataset
    dataset_kwargs = {}
    if args.paired:
        dataset_kwargs["nnue_cache_dir"] = args.nnue_cache_dir
    dataset = dataset_cls(
        args.bin, args.feature_set, args.batch_size, device=args.device, shuffle=False, **dataset_kwargs
    )
    ...
    eb_ds = dataset_cls(
        args.bin, args.feature_set, 8, device='cpu', shuffle=False, **dataset_kwargs
    )
    baseline_bin = eb_ds.nnue_bin_path if args.paired else args.bin
    baseline_ds = nnue_dataset.SparseBatchDataset(
        args.feature_set, baseline_bin, 8, cyclic=False, device='cpu'
    )
